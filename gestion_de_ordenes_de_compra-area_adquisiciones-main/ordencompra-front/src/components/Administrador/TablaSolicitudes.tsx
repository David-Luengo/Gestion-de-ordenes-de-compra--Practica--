...
import AsignarModal from "@modals/AsignarModal";
...
export default function TablaSolicitudes() {
  ...
  useEffect(() => {
    ...
    async function cargarSolicitudes() {
      ...
      try {
        ...
        const SolicitudResponse = await HttpClient.get(
          "solicitudes/todas_las_solicitudes/?&page=" +
            currentPage +
            "&pageSize=25" +
            "&proveedorRut=" +
            filtroRutProveedor +
  ...
  useEffect(() => {
    ...
    try {
      HttpClient.get("cotizaciones").then((response) => {
        setcotizacionesData(response.data);
      });
    } catch (error) {}
  }, [esAutenticacionValida, solicitudesData]);
  ...
  //   if (estado === 'En progreso' || estado === 'Aprobada' || estado === 'Rechazada') {
  //     return
  //   } else {
  //     try {
  //       const datos = {
  ...
  return (
    <>
      <section className="container mx-auto bg-white rounded-md p-2">
        <TitleHeader title="SOLICITUDES DISPONIBLES" />
      ...
      <AsignarModal
        isOpen={modalIsOpen.asignarSolicitud}
        idSolicitud={idSolicitud}
        onRequestClose={() => {
          setModalIsOpen((prevState) => ({
            ...prevState,
